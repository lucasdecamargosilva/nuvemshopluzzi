// Script carregado — Provador Virtual Luzzi Store (Nuvemshop)
(function () {
    ...
    // ===============================================
    const apiKey = "pl_live_a0741eb6832f6944cbae35b7e89387fe9ea3caa0942cb4cfe6f2c74229a24590";
    ...
    function init() {
        ...
        const fallbackContainers = [
            '.image-show', '.product__media-wrapper', '.product-gallery__media',
    ...
    // ─── PRODUTOS FORA DO PROVADOR (Luzzi) ────────────────────────────────────
    // A loja vende roupa, calcado e bolsa. O try-on so veste ROUPA.
    // (1) categoria pelo href do breadcrumb (imune a acento/caixa);
    // (2) nome do produto — pega o que escapa da categoria, ex.: "Bota Stretch",
    //     cadastrada em /novidades/ e nao em /calcados/.
    ...
    function plProdutoBloqueado() {
        try {
            ...
            var nome = (document.querySelector('h1.js-product-name, h1[itemprop="name"], .product-name h1, h1') || {}).textContent || document.title || '';
            if (PL_RE_BLOQ.test(nome)) return true;
        } catch (e) {}
        return false;
    }

    function plBoot() { if (plProdutoBloqueado()) { return; } init(); }
    if (document.readyState === 'complete') plBoot();
    else window.addEventListener('load', plBoot);
